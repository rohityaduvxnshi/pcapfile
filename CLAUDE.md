...
2. **Map** — `IcdImportDialog` (static frame in `forms/IcdImportDialog.ui`, combos/tree filled in code — see §10.14): tick field tables, set header-row index + **offset base (0/1)**, map columns → roles (Name/ByteOffset/DataType required; Length/Resolution/ResolutionExpression optional). Type labels resolve through `FieldCsvCodec::dataTypeFromLabel`. Message name from preceding heading or custom prefix; default port; auto payload length. Mappings save/load as named JSON **profiles** under `AppDataLocation/icd_mapping_profiles` (`IcdDocxImporter::saveProfile`/`loadProfile`/`availableProfiles`/`profileToJson`/`profileFromJson`).
   - **Auto-detect (`IcdDocxImporter::suggestMapping`)** — content-aware heuristic that makes mapping column-order-independent (handles byte-first, name-first, etc.). Pure/deterministic, no AI. Per table it: scores the first ≤6 rows for role keywords → **header row**; samples each column's *data* cells → **DataType** = most cells resolving via `dataTypeFromLabel` (header "type/format/encoding" confirms); **Offset** = mostly-numeric column with the widest increasing/distinct range (header "offset/position/byte" wins); **Length** = small-valued repeating numeric column (header "length/len/size/bytes"); **Name** = textual high-distinctness column (header "name/field/…" wins; numeric penalised; first-column tiebreak); Resolution/Expr by header keyword only; **offset base** from the offset column's min value (0→0-based, 1→1-based). Returns -1 for unsure roles. The dialog runs it automatically when the reference **table** changes (`onReferenceTableChanged`, guarded by `m_autoDetectedForTable` so header-row spin edits don't re-trigger), and on the **"Auto-detect columns"** button (`onAutoDetectClicked`); content guesses (≥0) override the keyword pre-fill, which is the fallback. Loading a profile sets `m_autoDetectedForTable` to suppress an immediate overwrite. Always user-overridable; the review tree is the safety net.
