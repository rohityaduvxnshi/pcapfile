...
// stay at their defaults and are ignored by the NMEA path.
//
// Simulator: every row also carries a VALUE — the token transmitted at that
// comma position when the sentence is built (FieldDefinition.sendValueText).
// Disabled/blank rows become null fields (",,") in the sentence.
...
private:
    void refreshTable();
    // Custom mode helpers.
    void addCustomRow(int fieldIndex, const QString& name, int valueKind, const QString& value);
    ...
    struct RowState
    {
        bool    enabled;
        QString customLabel;
        QString valueText;   // simulator: the token to transmit
